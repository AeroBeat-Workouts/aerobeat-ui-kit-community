...
func apply_visual_state(state: Dictionary, is_hybrid_world: bool, badge_tokens: Dictionary, button_style_config: AeroUiGlassPrimaryButtonConfig, shell_tint: Color, accent_color: Color = DEFAULT_ACCENT) -> void:
	...
	var accent := accent_color if active else Color(1.0, 1.0, 1.0, 1.0)
	var fill_alpha: float = float(badge_tokens["fill_alpha"]) + float(_state_value(button_style_config, is_hybrid_world, "rest", "fill_delta", 0.0))
	var border_alpha: float = float(badge_tokens["border_alpha"]) + float(_state_value(button_style_config, is_hybrid_world, "rest", "border_delta", 0.0))
	var shadow_alpha: float = float(_state_value(button_style_config, is_hybrid_world, "rest", "shadow_alpha", 0.0))
	var shadow_size: int = int(_state_value(button_style_config, is_hybrid_world, "rest", "shadow_size", 0))

	if hovered:
		fill_alpha = maxf(fill_alpha, float(badge_tokens["fill_alpha"]) + _state_value(button_style_config, is_hybrid_world, "hover", "fill_delta", 0.0))
		border_alpha = maxf(border_alpha, float(badge_tokens["border_alpha"]) + _state_value(button_style_config, is_hybrid_world, "hover", "border_delta", 0.0))
		shadow_alpha = maxf(shadow_alpha, _state_value(button_style_config, is_hybrid_world, "hover", "shadow_alpha", 0.0))
		shadow_size = max(shadow_size, int(_state_value(button_style_config, is_hybrid_world, "hover", "shadow_size", 0)))
	if pressed or toggled:
		fill_alpha = maxf(fill_alpha, float(badge_tokens["fill_alpha"]) + _state_value(button_style_config, is_hybrid_world, "pressed", "fill_delta", 0.0))
		border_alpha = maxf(border_alpha, float(badge_tokens["border_alpha"]) + _state_value(button_style_config, is_hybrid_world, "pressed", "border_delta", 0.0))
		shadow_alpha = maxf(shadow_alpha, _state_value(button_style_config, is_hybrid_world, "pressed", "shadow_alpha", 0.0))
		shadow_size = max(shadow_size, int(_state_value(button_style_config, is_hybrid_world, "pressed", "shadow_size", 0)))

	var body_style := _build_action_stylebox(fill_alpha, accent, active or hovered, border_alpha, shadow_alpha, shadow_size, is_hybrid_world, badge_tokens, button_style_config, shell_tint)
...
func _build_action_stylebox(fill_alpha: float, accent: Color, is_emphasized: bool, border_alpha: float, shadow_alpha: float, shadow_size: int, is_hybrid_world: bool, badge_tokens: Dictionary, button_style_config: AeroUiGlassPrimaryButtonConfig, shell_tint: Color) -> StyleBoxFlat:
	var style := StyleBoxFlat.new()
	var fill_color := Color(1.0, 1.0, 1.0, clampf(fill_alpha, 0.0, 0.44 if is_hybrid_world else 0.24))
	if not is_hybrid_world:
		fill_color = fill_color.lerp(Color(shell_tint.r, shell_tint.g, shell_tint.b, fill_color.a), 0.16)
...
func _state_value(button_style_config: AeroUiGlassPrimaryButtonConfig, is_hybrid_world: bool, phase: String, key: String, default_value: Variant) -> Variant:
	return _button_state(button_style_config, is_hybrid_world, phase).get(key, default_value)
